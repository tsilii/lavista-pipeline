...
def auto_rotate_image(image_bytes: bytes) -> bytes:
    ...
    WhatsApp photos often have EXIF rotation tags that make them appear
    rotated when read as raw bytes. This fixes that.
    Returns corrected image bytes in the same format.
    """
    try:
        img = Image.open(io.BytesIO(image_bytes))
        # ImageOps.exif_transpose reads EXIF orientation tag and rotates accordingly
